...
class AdminController extends Controller
{
    ...
    public function del_category(Request $request)
    {
        ...
        if ($valid) {
            ...
            if ($pro->count() > 0) {
                $request->session()->flash('msg', 'Не получается удалить категорию т.к. она используется');
                $request->session()->flash('msgst', 'Опасно');
            } else {
                $image = $cate->image;
                Storage::delete('public/images/' . $image);
                $cate->delete();
                $request->session()->flash('msg', 'Удалено...');
                $request->session()->flash('msgst', 'Успешно');
            }
        }
